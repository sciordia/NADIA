...
One boxplot per normalization method showing the per-protein MAD
distribution (MAD averaged across groups for each protein). Lower values
indicate better within-group consistency. With `diff = TRUE`, shows %
reduction vs `baseline` as a bar chart.
