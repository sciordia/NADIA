...
One boxplot per normalization method showing the per-protein CV
distribution (CV averaged across groups for each protein). Lower and
less spread values indicate better within-group consistency. With
`diff = TRUE`, shows % reduction vs `baseline` as a bar chart.
