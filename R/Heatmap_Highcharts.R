...
# =============================================================================
# Heatmap Interactivo con heatmaply
# =============================================================================

#' Crear heatmap interactivo de proteómica con heatmaply
#'
#' Genera un heatmap interactivo usando plotly/heatmaply con dendrogramas,
#' anotaciones y tooltips interactivos.
#'
#' @param data Data frame en formato largo con columnas requeridas
#' @param mode Modo de filtrado: "all", "any", o "target"
#' @param alpha Umbral de significancia para filtrado (default: 0.05)
#' @param comparison Nombre de la comparación para modo "target" (ej: "B-A")
#' @param feature_ids Vector de FeatureIDs específicos (ignora mode/alpha si se proporciona)
#' @param row_annotation Anotaciones para filas (ruta TSV o data.frame con FeatureID)
#' @param row_annotation_cols Columnas a usar como anotaciones de fila
#' @param row_annotation_palette Lista de paletas para cada columna de anotación
#' @param row_order_by Orden de filas: "clustering", columna(s) de anotación, o "adjP"
#' @param scale_data Tipo de escalado: "none", "row", "column" (default: "row")
#' @param sample_order Orden de muestras: "clustering", "condition", o vector personalizado
#' @param condition_order Orden de condiciones cuando sample_order = "condition"
#' @param cluster_rows Aplicar clustering a filas (default: TRUE)
#' @param cluster_columns Aplicar clustering a columnas (default: TRUE)
#' @param show_row_names Mostrar nombres de filas (default: TRUE si <= 100 proteínas)
#' @param show_column_names Mostrar nombres de columnas (default: TRUE)
#' @param palette_value Paleta para valores del heatmap
#' @param palette_annotation Paleta para anotación de Condition
#' @param reverse_palette Invertir paleta de valores (default: FALSE)
#' @param row_title Título para las filas (eje Y)
#' @param column_title Título para las columnas (eje X)
#' @param show_annotation Mostrar anotación de Condition (default: TRUE)
#' @param heatmap_title Título principal del heatmap
#' @param fontsize_row Tamaño de fuente de nombres de fila (default: 10)
#' @param fontsize_col Tamaño de fuente de nombres de columna (default: 10)
#' @param plot_width Ancho del plot en píxeles (default: NULL, auto)
#' @param plot_height Alto del plot en píxeles (default: NULL, auto)
#' @param dendrogram Mostrar dendrogramas: "both", "row", "column", "none" (default: "both")
#' @param show_legend Mostrar leyenda de colores (default: TRUE)
#' @param na_color Color para valores NA (default: "grey50")
#'
#' @return Objeto plotly/heatmaply interactivo
#'
#' @examples
#' \dontrun{
#' # Heatmap interactivo básico
#' hm_int <- proteomics_heatmap_interactive(
#'   data = hm_input,
#'   mode = "target",
#'   comparison = "B-A",
#'   scale_data = "row"
...
#' hm_int
#'
#' # Con anotaciones de fila
#' hm_int <- proteomics_heatmap_interactive(
#'   data = hm_input,
#'   mode = "target",
#'   comparison = "B-A",
#'   row_annotation = "annotations.tsv",
#'   row_annotation_cols = c("Specie", "Process")
#' )
#' }
#'
#' @export
proteomics_heatmap_interactive <- function(data,
                                           mode = c("all", "any", "target"),
                                           alpha = 0.05,
                                           comparison = NULL,
                                           feature_ids = NULL,
  ...
  # Verificar que heatmaply está instalado

  if (!requireNamespace("heatmaply", quietly = TRUE)) {
    stop("El paquete 'heatmaply' es necesario. Instalar con: install.packages('heatmaply')")
  }
  ...
  # ---------------------------------------------------------------------------
  # 1) Preparar datos usando la función existente
  # ---------------------------------------------------------------------------
  ...
  # Determinar número de proteínas y muestras
  n_proteins <- nrow(mat)
  n_samples <- ncol(mat)

  # Auto-decidir si mostrar nombres de filas
  if (is.null(show_row_names)) {
    show_row_names <- n_proteins <= 100
  }

  # ---------------------------------------------------------------------------
  # 3) Preparar anotaciones de columna (Condition)
  # ---------------------------------------------------------------------------
  ...
  if (show_annotation) {
    # Obtener información de condición por muestra
    sample_info <- hm_data %>%
      select(SampleID, Condition) %>%
      distinct()

    # Ordenar según columnas de la matriz
  ...
  # ---------------------------------------------------------------------------
  # 4) Preparar anotaciones de fila
  # ---------------------------------------------------------------------------
  ...
  if (!is.null(row_annotation)) {
    # Cargar datos de anotación
    if (is.character(row_annotation)) {
      if (file.exists(row_annotation)) {
        row_annot_data <- readr::read_tsv(row_annotation, show_col_types = FALSE)
      } else {
        warning("Archivo de anotación no encontrado: ", row_annotation)
      }
    } else if (is.data.frame(row_annotation)) {
      row_annot_data <- row_annotation
    }

    if (!is.null(row_annot_data)) {
      # Verificar columna FeatureID
      if (!"FeatureID" %in% names(row_annot_data)) {
        warning("El data.frame de anotación debe contener una columna 'FeatureID'")
        row_annot_data <- NULL
      }
    }
  }
  ...
  if (!is.null(row_annot_data)) {
    # Determinar columnas de anotación
    if (is.null(row_annotation_cols)) {
      all_annot_cols <- setdiff(names(row_annot_data), "FeatureID")
      row_annotation_cols <- all_annot_cols
    } else {
      all_annot_cols <- names(row_annot_data)
      missing_cols <- setdiff(row_annotation_cols, all_annot_cols)
      if (length(missing_cols) > 0) {
        warning("Columnas de anotación no encontradas: ", paste(missing_cols, collapse = ", "))
        row_annotation_cols <- intersect(row_annotation_cols, all_annot_cols)
    ...
    if (length(row_annotation_cols) > 0) {
      # Filtrar solo FeatureIDs presentes en la matriz
      row_annot_data <- row_annot_data[row_annot_data$FeatureID %in% rownames(mat), , drop = FALSE]
      ...
      # Crear data frame de anotación de fila
      row_side_df <- row_annot_data %>%
        select(FeatureID, all_of(row_annotation_cols)) %>%
        as.data.frame()
      rownames(row_side_df) <- row_side_df$FeatureID
      row_side_df$FeatureID <- NULL

      # Reordenar según filas de la matriz
      row_side_colors <- row_side_df[rownames(mat), , drop = FALSE]

      # Preparar paletas de colores para cada anotación
      row_side_palette <- list()
      for (col in row_annotation_cols) {
        all_levels <- unique(row_side_colors[[col]])
        real_levels <- sort(all_levels[all_levels != "NA"])
        ...
        # Añadir blanco para "NA"
        if ("NA" %in% all_levels) {
          real_colors <- c(real_colors, "NA" = "#FFFFFF")
        }

        row_side_palette[[col]] <- real_colors
      }
    }
  }

  # ---------------------------------------------------------------------------
  # 5) Configurar clustering y ordenamiento de filas
  # ---------------------------------------------------------------------------
  ...
  if (!is.null(row_order_by)) {
    if (identical(row_order_by, "clustering")) {
      ...
      if (!is.null(row_side_colors)) {
        valid_order_cols <- c(valid_order_cols, names(row_side_colors))
      }
      if ("adjP" %in% names(hm_data)) {
        valid_order_cols <- c(valid_order_cols, "adjP")
      }
      ...
      if (length(order_cols) > 0) {
        ...
        order_df <- data.frame(FeatureID = rownames(mat))

        # Añadir columnas de anotación
        if (!is.null(row_side_colors)) {
          for (col in order_cols[order_cols %in% names(row_side_colors)]) {
            order_df[[col]] <- row_side_colors[order_df$FeatureID, col]
          }
        }

        # Añadir adjP si se necesita
        if ("adjP" %in% order_cols && "adjP" %in% names(hm_data)) {
          adjp_data <- hm_data %>%
            select(FeatureID, adjP) %>%
            distinct()
        ...
        # Ordenar
        order_syms <- rlang::syms(order_cols)
        order_df_sorted <- order_df %>%
          arrange(!!!order_syms)

        # Crear orden para filas
        row_order <- match(order_df_sorted$FeatureID, rownames(mat))
        Rowv <- FALSE
        mat <- mat[row_order, , drop = FALSE]

        # Reordenar anotaciones de fila
        if (!is.null(row_side_colors)) {
          row_side_colors <- row_side_colors[row_order, , drop = FALSE]
        }
      }
    }
  } else {
  ...
  # ---------------------------------------------------------------------------
  # 6) Configurar clustering de columnas
  # ---------------------------------------------------------------------------

  Colv <- NULL
  col_dend <- cluster_columns
  ...
  # ---------------------------------------------------------------------------
  # 7) Obtener paleta de colores para valores
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # 8) Configurar dendrograma
  # ---------------------------------------------------------------------------

  dendrogram_final <- dendrogram
  if (!row_dend && dendrogram %in% c("both", "row")) {
    dendrogram_final <- if (col_dend) "column" else "none"
  }
  if (!col_dend && dendrogram %in% c("both", "column")) {
    dendrogram_final <- if (row_dend) "row" else "none"
  }
  ...
  heatmaply_args <- list(
    x = mat,
    colors = value_colors,
    dendrogram = dendrogram_final,
    Rowv = if (isFALSE(Rowv)) FALSE else NULL,
    Colv = if (isFALSE(Colv)) FALSE else NULL,
  ...
  if (!is.null(row_side_colors)) {
    heatmaply_args$row_side_colors <- row_side_colors
    heatmaply_args$row_side_palette <- row_side_palette
  }

  # Añadir dimensiones si se especifican
  if (!is.null(plot_width)) {
    heatmaply_args$width <- plot_width
  }
  if (!is.null(plot_height)) {
    heatmaply_args$height <- plot_height
  }

  # Crear heatmap
  hm <- do.call(heatmaply::heatmaply, heatmaply_args)
...
# =============================================================================
# EJEMPLOS DE HEATMAP INTERACTIVO (heatmaply)
# =============================================================================
...
# --- Heatmap interactivo con clustering ---
# hm_int_cluster <- proteomics_heatmap_interactive(
#   data = hm_input,
#   mode = "any",
#   scale_data = "row",
#   cluster_rows = TRUE,
...
# --- Heatmap interactivo con anotaciones de fila ---
# hm_int_annot <- proteomics_heatmap_interactive(
#   data = hm_input,
#   mode = "target",
#   comparison = "B-A",
...
# --- Heatmap interactivo ordenado por condición ---
# hm_int_cond <- proteomics_heatmap_interactive(
#   data = hm_input,
#   mode = "target",
#   comparison = "B-A",
#   scale_data = "row",
...
#   dendrogram = "row"  # Solo dendrograma de filas
# )
# hm_int_cond

# --- Heatmap interactivo con paleta personalizada ---
# hm_int_pal <- proteomics_heatmap_interactive(
#   data = hm_input,
#   mode = "any",
#   scale_data = "row",
#   palette_value = "viridis::plasma",
#   palette_annotation = "brewer:Set1",
#   heatmap_title = "Interactive Proteomics Heatmap"
# )
# hm_int_pal

# --- Heatmap interactivo con dimensiones personalizadas ---
# hm_int_size <- proteomics_heatmap_interactive(
#   data = hm_input,
#   mode = "target",
#   comparison = "B-A",
#   scale_data = "row",
