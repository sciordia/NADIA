...
img[src$="Input_Formats_Workflow.svg"] {
  width: 100% !important;
...
img[src$="Input_Formats_Workflow.svg"].input-format-clickable {
  cursor: zoom-in;
}
.figure-zoom-hint {
...
img[src$="Spectronaut_Input_Structure.svg"],
#fig\:diann-input-structure + img,
img[src$="DIANN_Input_Structure.svg"],
#fig\:tmt-input-structure + img,
img[src$="TMT_Input_Structure.svg"],
#fig\:lfq-input-structure + img,
img[src$="LFQ_Input_Structure.svg"] {
  width: 100% !important;
  max-width: none !important;
}
@media screen and (min-width: 992px) {
...
(function () {
  ...
  if (!source || source.tagName !== "IMG") {
    source = document.querySelector('img[src$="Input_Formats_Workflow.svg"]');
  }
  if (!source) return;
...
document.addEventListener("DOMContentLoaded", function () {
  // BiocStyle numbers figures and leaves an anchor before each image; pkgdown
  // emits no such anchor, so each entry also carries the file to fall back on.
  var figures = [
    ["fig:spectronaut-input-structure", "Spectronaut_Input_Structure.svg"],
    ["fig:diann-input-structure",       "DIANN_Input_Structure.svg"],
    ["fig:tmt-input-structure",         "TMT_Input_Structure.svg"],
    ["fig:lfq-input-structure",         "LFQ_Input_Structure.svg"]
  ];
  var sources = figures.map(function (entry) {
    var anchor = document.getElementById(entry[0]);
    var image = anchor ? anchor.nextElementSibling : null;
    if (image && image.tagName === "IMG") return image;
    return document.querySelector('img[src$="' + entry[1] + '"]');
  }).filter(Boolean);
