...
To combine the clusters with the differential-abundance statistics in a single
table, for example as input to a functional or enrichment analysis, use
`deps_with_clusters()` on the two results in memory, or
`nadia_deps_with_clusters()` on a stored project. Both are described in
