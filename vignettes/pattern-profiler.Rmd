...
.horizontal-scroll > table:has(> caption) {
  margin-left: 65px !important;
  margin-right: 0 !important;
}

.cluster-summary-table .horizontal-scroll > table {
...
<summary>Show the code used to summarise the experimental design</summary>

```{r example-design, results='hide'}
...
```{r example-design-table, echo=FALSE}
knitr::kable(
  design_table,
  caption = "Number of samples available for each condition."
)
...
<details class="code-details">
<summary>Show the code used to summarise the Pattern Profiler run</summary>

```{r run-overview, eval=have_pattern_profiler, results='hide'}
...
```{r run-overview-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  run_overview,
  caption = "Overview of the Pattern Profiler run."
)
...
<details class="code-details">
<summary>Show the code used to compare the filtering strategies</summary>

```{r filter-counts, results='hide'}
...
```{r filter-counts-table, echo=FALSE}
knitr::kable(
  filter_counts,
  caption = "Number of proteins selected by three filtering strategies."
)
...
<details class="code-details">
<summary>Show the code used to prepare the cluster-selection diagnostics</summary>

```{r selection-metrics, eval=have_pattern_profiler, results='hide'}
...
```{r selection-metrics-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  selection_table,
  row.names = FALSE,
  caption = "Cluster-selection diagnostics for c = 2 to 5."
...
is converted to a row-wise z-score, these values describe the **relative profile
of each protein across the ordered conditions**; they must not be interpreted as
absolute protein abundance.

<details class="code-details">
<summary>Show the code used to preview the long-format output</summary>

```{r long-output, eval=have_pattern_profiler, results='hide'}
knitr::kable(
...
```{r long-output-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  head(pp$long_output, 6),
  digits = 3,
  caption = "First rows of the long-format Pattern Profiler output."
...
<details class="code-details">
<summary>Show the code used to summarise soft-clustering assignments</summary>

```{r membership-summary, eval=have_pattern_profiler, results='hide'}
...
```{r membership-summary-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  membership_overview,
  caption = "Summary of the soft-clustering assignments."
)
...
<details class="code-details">
<summary>Show the code used to summarise cluster membership</summary>

```{r cluster-summary, eval=have_pattern_profiler, results='hide'}
...
```{r cluster-summary-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  cluster_table,
  caption = "Membership summary for each cluster."
)
...
<details class="code-details">
<summary>Show the code used to compare membership thresholds</summary>

```{r membership-thresholds, eval=have_pattern_profiler, results='hide'}
...
```{r membership-thresholds-table, eval=have_pattern_profiler, echo=FALSE}
knitr::kable(
  threshold_table,
  caption = "Effect of applying stricter membership thresholds."
)
...
To combine Pattern Profiler assignments with differential-abundance statistics
in a single table, for example as input to a downstream functional or enrichment
analysis, use `deps_with_clusters()` on the two results in memory or
`nadia_deps_with_clusters()` on a stored project. Both functions are described
in `vignette("results-and-export", package = "NADIA")`.
