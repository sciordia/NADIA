...
# fig.ext has to be set alongside dev: pkgdown fixes the figure extension at
# "png" for articles, so with dev = "svg" alone knitr writes SVG documents into
# files named *.png and the browser refuses to decode them. BiocStyle never
# showed it, because a self-contained render embeds the image and the file name
# stops mattering.
...
.lightbox-figure.pca-covariates-figure {
  width: 110%;
  margin-left: -5%;
}
.lightbox-figure.tmt-benchmark-table {
  width: 110%;
  margin-left: -5%;
}
.tmt-pca-figure {
...
<img alt="Enlarged diagram" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" />
</div>
...
sample separation. Click the figure to enlarge it:

```{r pca-cov, eval = run_bc, message = FALSE, fig.show = "hide"}
...
::: {.pca-covariates-figure .lightbox-figure}
```{r pca-cov-styled, eval = run_bc, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 7, fig.height = 6.2, fig.cap = "PCA of the simulated example after correction, coloured in turn by each biological or technical variable.", fig.alt = "PCA coloured by condition, digestion, injection and subject using distinct colours and shapes"}
pca_colors <- c(
    A  = "#0072B2", B  = "#D55E00", D  = "#009E73",
    d1 = "#CC79A7", d2 = "#E69F00",
    i1 = "#56B4E9", i2 = "#333333",
    s1 = "#8C564B", s2 = "#6A3D9A", s3 = "#E41A1C", s4 = "#607D8B")

pca_shapes <- c(
    A = 16, B = 17, D = 15,
    d1 = 16, d2 = 17,
    i1 = 16, i2 = 17,
    s1 = 16, s2 = 17, s3 = 15, s4 = 18)

pca_grid$grid +
    ggplot2::aes(shape = Value) +
    ggplot2::scale_color_manual(
        values = pca_colors,
        breaks = names(pca_colors),
        name = NULL) +
    ggplot2::scale_shape_manual(
        values = pca_shapes,
        breaks = names(pca_shapes),
        guide = "none") +
    ggplot2::guides(
