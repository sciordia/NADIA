...
No `batch_covariates` are used here, and the reason is a limitation rather than a
preference: on this dataset the protected model cannot be fitted. BERT aborts for
every ComBat mode, and the `limma` algorithm fails earlier still.

```{r tmt-protected-attempt, eval = FALSE}
# Not evaluated: this call fails on this dataset.
#   ComBat, modes 1-4 -> "NA/NaN/Inf in foreign function call (arg 1)"
#   limma             -> "design must be a numeric matrix"
process_proteomics(
    tmt, covariate_df = tmt_cov,
    batch_correct = TRUE, batch_column = "Mix",
...
This is not the insufficient replication described for the simulated example.
Every combination of mix and condition contains four samples, no protein group is
unfittable within either grouping, and the normalised assay contains a single
missing value across 2,000 protein groups and 24 samples; removing that protein
does not change the outcome. The failure occurs inside BERT's handling of
covariates.

The guidance given earlier still stands: when a biological variable is known and
the design permits it, that variable should be protected. This dataset is an
example of the remaining case, in which the protected model cannot be fitted at
all. Because condition is represented equally in both mixes, comparatively little
condition-associated variation is at risk from an unprotected correction here,
but that balance mitigates the consequences rather than justifying the choice.
It is also why the design tables of step 1 and the checks that follow the
correction are necessary rather than optional.
...
round(100 * cbind(
    ...
    corrected   = directional_performance(tmt_bc,  "D-A")), 1)
```
...
sweep <- do.call(rbind, lapply(c(0, 0.2, 0.3, 0.5, 1), function(thr)
    data.frame(logFC_threshold = thr,
               assay           = c("uncorrected", "corrected"),
               rbind(score(tmt_res, thr), score(tmt_bc, thr)))))
rownames(sweep) <- NULL
sweep
```

At a threshold of zero the uncorrected analysis appears to be the better of the
two, but this is low statistical power presented as specificity: the mix effect
