...
@media screen and (min-width: 992px) {
  ...
  img[src$="Combo_Classification_Examples.svg"] {
    width: 106% !important;
    max-width: none !important;
    margin-left: -3%;
  }
...
img[src$="Combo_Classification_Examples.svg"].combo-classification-clickable {
  cursor: zoom-in;
}
.figure-zoom-hint {
...
(function () {
  ...
  // pkgdown emits no such anchor, so fall back to the file itself.
  if (!source || source.tagName !== "IMG") {
    source = document.querySelector('img[src$="Combo_Classification_Examples.svg"]');
  }
  if (!source) return;
