...
missing values. Incomplete proteins have a lower median observed intensity than
complete proteins in both matrices, with a difference of 1.9 log~2~ units in
...
The abundance estimates and their uncertainty are converted into **precision
weights** for differential analysis. Values reconstructed from limited evidence
therefore contribute less to the statistical model than values supported by
consistent measurements. This allows information to be recovered from missing
values without treating every estimate as if it were measured with equal
precision.
